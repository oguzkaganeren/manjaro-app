...
import { ScrollView, Dimensions, Linking, Image, ViewStyle, useWindowDimensions } from 'react-native';
	...
	const ChooseHeader = (subprops) => (
		<Layout {...subprops}>
			<Layout style={[globalStyle.sideContainer, style]}>
				<Text category="h4">
					{jsonData.name.substring(0, 7)}
					{jsonData.name.length > 7 ? '...' : ''}
				</Text>

				<Button style={[eva.style!.download, style]} onPress={showModal} status="primary" accessoryLeft={DownloadIcon}>
		...
		<Layout style={globalStyle.container}>
				...
				<Card disabled header={ChooseHeader}>
					{jsonData.Youtube ? (
						...
						<Image
							...
							source={{
								uri: 'https://hacked.manjaro.org/img/editions/' + jsonData.Screenshot,
			...
			>
				<Card disabled>
					...
					<Text></Text>
					<Text appearance="hint">Minimal SHA1: {jsonData.Download_Minimal_x64_Checksum}</Text>
					<Text
						style={{ color: 'green' }}
						category="p1"
						onPress={() => {
